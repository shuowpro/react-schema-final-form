...
var getWidget = function getWidget(_ref) {
...
var renderField = function renderField(_ref2) {
  var _ref2$schema = _ref2.schema,
      schema = _ref2$schema === undefined ? {} : _ref2$schema,
      ...
      _ref2$mutators = _ref2.mutators,
      mutators = _ref2$mutators === undefined ? {} : _ref2$mutators;
...
var resolveRef = function resolveRef(uri, wrapWith, schema) {
  uri = uri.replace('#/', '');
  var tokens = [].concat(toConsumableArray(wrapWith.split('/').filter(function (v) {
    return v !== '';
  })), toConsumableArray(uri.split('/')));
...
var compileSchema = function compileSchema() {
  var schema = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var wrapWith = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  ...
  if (Array.isArray(schema)) {
    return schema.map(function (item) {
      return compileSchema(item, wrapWith, root);
    });
  }
  return schema;
};

var compileRefs = function compileRefs(_ref) {
  var _ref$schema = _ref.schema,
      schema = _ref$schema === undefined ? {} : _ref$schema,
      _ref$wrapWith = _ref.wrapWith,
      wrapWith = _ref$wrapWith === undefined ? '' : _ref$wrapWith;

  var compiled = compileSchema(schema, wrapWith, schema);
  var originSchema = wrapWith.split('/').filter(function (v) {
    return v !== '';
...
var setError = function setError(error, schema, res) {
  ...
  var type = findTypeInSchema(schema, dataPath.split('.'));

  var errorMessage = type === "array" ? { _error: error.message } : error.message;

  lodash_set(res, dataPath, errorMessage);
  return res;
};
...
var renderSchema = function renderSchema(_ref) {
  var _ref$schema = _ref.schema,
      schema = _ref$schema === undefined ? {} : _ref$schema,
      _ref$theme = _ref.theme,
      theme = _ref$theme === undefined ? {} : _ref$theme,
      _ref$mutators = _ref.mutators,
      mutators = _ref$mutators === undefined ? {} : _ref$mutators;
...
export { compileRefs, renderField, renderObjectProperties, buildSyncValidation };
