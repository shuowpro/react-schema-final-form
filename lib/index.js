...
var setError = function setError(error, schema, res) {
  // remove the first '.', convert '.foo.bar' to 'foo.bar'
  var dataPath = error.dataPath.charAt(0) === '.' ? error.dataPath.slice(1) : error.dataPath;
  console.log(error.dataPath);
  lodash_set(res, dataPath, error.message);
  return res;
};
...
var buildSyncValidation = function buildSyncValidation(schema) {
  ...
  return function (values) {
    ...
    var errors = ajv.errors.reduce(function (res, error) {
      return setError(error, schema, res);
    }, {});
    console.log(errors);
    return errors;
  };
...
var getWidget = function getWidget(schema, theme) {
  if (schema.widget) {
...
var getRef = function getRef(ref, rootSchema) {
  var path = ref.replace("#/", "").replace("/", ".");
  return lodash_get(rootSchema, path);
};
...
var renderField = function renderField(_ref) {
  var schema = _ref.schema,
      ...
      fieldName = _ref$fieldName === undefined ? '' : _ref$fieldName,
      _ref$required = _ref.required,
      required = _ref$required === undefined ? false : _ref$required,
      rest = objectWithoutProperties(_ref, ['schema', 'rootSchema', 'theme', 'fieldName', 'required']);

  if (schema.$ref) {
    var $ref = schema.$ref,
        others = objectWithoutProperties(schema, ['$ref']);

    var newSchema = getRef($ref, rootSchema);
    if (!newSchema) throw new Error("Cannot resolve the $ref: " + schema.$ref);
    return renderField({
      schema: _extends({}, others, newSchema),
...
var ReactFinalSchemaForm$1 = function (_React$Component) {
  inherits(ReactFinalSchemaForm, _React$Component);

  function ReactFinalSchemaForm() {
    classCallCheck(this, ReactFinalSchemaForm);
    return possibleConstructorReturn(this, (ReactFinalSchemaForm.__proto__ || Object.getPrototypeOf(ReactFinalSchemaForm)).apply(this, arguments));
  }

  createClass(ReactFinalSchemaForm, [{
    key: 'getChildContext',
    ...
    value: function render() {
      ...
      var validate = buildSyncValidation(schema);
      return children({
  ...
  return ReactFinalSchemaForm;
}(Component);

ReactFinalSchemaForm$1.propTypes = {
  /**
...
ReactFinalSchemaForm$1.childContextTypes = {
  reactFinalSchemaForm: propTypes.object
};

var isReuqired = function isReuqired(_ref) {
...
var renderObjectProperties = function renderObjectProperties(_ref2) {
  var schema = _ref2.schema,
      rootSchema = _ref2.rootSchema,
      theme = _ref2.theme,
      _ref2$fieldName = _ref2.fieldName,
      fieldName = _ref2$fieldName === undefined ? '' : _ref2$fieldName;
  return Object.keys(schema.properties).map(function (propertyName) {
    return {
...
export { renderField, renderObjectProperties, buildSyncValidation };
export default ReactFinalSchemaForm$1;
